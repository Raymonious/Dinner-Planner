...
describe("TW3.1 DinnerModel notifies its observers [test](/tw3.1.html)", function tw3_1_20() {
  this.timeout(200000);
 ...
 function callMethodTwiceWithValueAndTestObserver(method, field, value, value2=value, expectedValue1=value, expectedValue2=value2) {
  ...
  it("setNumberOfGuests calls notifyObservers", function tw3_1_20_1() {
      callMethodTwiceWithValueAndTestObserver("setNumberOfGuests", "numberOfGuests", 99);
  });

    it("addToMenu calls notifyObservers if the dish not in the menu", function tw3_1_20_2(){  
        callMethodTwiceWithValueAndTestObserver("addToMenu", "dishes", {id: 1}, {id: 1}, [{id:1}], [{id:1}, {id:1}]);
    });

    it("removeFromMenu calls nitifyObservers if the dish is in the menu",  function tw3_1_20_3(){
        model.addToMenu({id:1});
      // dish 1 is now added to menu
      callMethodTwiceWithValueAndTestObserver("removeFromMenu", "dishes", {id: 1}, {id: 1}, [], []);
  });

  it("setCurrentDish calls notifyObservers if the current dish changes", async function tw3_1_20_2() {
    ...
    try {
      ...
      expect(timesObserverNotified, "expected notification from setCurrentDish").to.equal(1);

        /*
      //checkPayload("setCurrentDish", 1, payload);
      await model.currentDishPromiseState.promise;
      expect(timesObserverNotified, "expected notification from setCurrentDish after promise is resolved").to.equal(1);
      */

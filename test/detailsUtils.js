...
async function makeRender(DetailsPresenter, React, render, h, model, propsHistory){
    ...
    const Root={
        ...
        render(){
            return <DetailsPresenter model={vueModel.model=this.rootModel}/>;
        },
    };
    ...
    await withMyFetch(
        myDetailsFetch,
        ()=>render(React.useState?<Guard><DetailsPresenter model={model}/></Guard>:<Root model={model}/>, div)
        );
    return {div, vueModel};
}
...
//        currentDishPromiseState:{ },
    ...
    it("DetailsPresenter resolves a promise when the current dish is set", async function tw_3_2_50_2_1(){
        ...
        modelTarget.currentDish=null;
        await doRender();
        const {changeGuests, addDish, removeDish, noCurrentDishPromise, changeCurrentDish, dummyNotification}=require("./payloadUtils.js");
        await withMyFetch(
            myDetailsFetch, 
...
//        await new Promise(resolve => setTimeout(resolve));
    ...
    it("Details presenter does not update view on dummy observer notification",  async function tw_3_2_50_2_5(){
        ...
        await doRender();
        propsHistory.length=0;
        const {changeGuests, addDish, removeDish, noCurrentDish, changeCurrentDish, dummyNotification}=require("./payloadUtils.js");
...
function checkModelFetch(){
    it("DinnerModel setCurrentDish does not initiate a fetch", function tw_3_2_70_5(){
        const DM= require("../src/" + X + "DinnerModel.js").default;
        try{
            new DM().setCurrentDish(314);
        }catch(e){

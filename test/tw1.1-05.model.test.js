...
describe("TW1.1 DinnerModel", function tw1_1_05() {
    ...
    beforeEach(function  tw1_1_05_beforeEach() {
        ...
        // we pass a few random extra parameters, to make sure that the model does not use more parameters than nr-guests and dishes
        try{
            model = new DinnerModel(2, [], 7, 7, 7);
        }catch(e){
            console.error(e);
        }
    });
    ...
    it("number of guests can only be set to a positive integer", function  tw1_1_05_1(){
        ...
        expect(model.numberOfGuests, "error in constructor, number of guests not properly initialized").to.equal(2);
        expect(model.dishes, "error in constructor, dishes  not properly initialized to empty array").to.be.an("array");
        expect(model.dishes.length, "error in constructor, dishes  not properly initialized to empty array").to.equal(0);
    ...
    it("can set current dish", function  tw1_1_05_3(){
        expect(model.currentDish, "currentDish expected to be initially undefined and not depend on constructor parameters").to.be.undefined;
        ...
        try{
            ...
            expect(model.currentDish, "expected current dish to be 1 after haing set it to 1").to.equal(1);
            
            model.setCurrentDish(3);
            expect(model.currentDish, "expected current dish to be 3 after haing set it to 3").to.equal(3);
        }finally{

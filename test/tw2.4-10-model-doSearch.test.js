...
describe("TW2.4 Promise State in Model: search [test](/tw2.4.1.html)", function tw2_4_10() {
  ...
  this.beforeEach(function tw2_4_10_beforeEach() {
    ...
    it("Model initializes searchParams (set searchParams in constructor to enable)", function tw2_4_10_1() {
        this._runnable.title= "Model sets the searchParams for search query and type";
        ...
        expect(JSON.stringify(model.searchParams), "searchParams expected to be initially an empty object and not depend on constructor parameters").to.equal(JSON.stringify({}));
    });
    ...
    it("Model initializes searchResultsPromiseState in constructor", async function tw2_4_10_2() {
        expect(
            model,
            "Model must have a searchResultsPromiseState"
        ).to.have.property("searchResultsPromiseState");
        expect(JSON.stringify(model.searchResultsPromiseState), "searchResultsPromiseState expected to be initially an empty object and not depend on constructor parameters").to.equal(
            JSON.stringify({})
        );
    });
  ...
  it("doSearch performs a search with the given params and resolves the promise into searchResultsPromiseState", async function tw2_4_10_2() {
      withMyFetch(mySearchFetch, ()=>model.doSearch({query:"ice cream", type:"dessert"})); 
    ...
    expect(
      ...
      expect(mySearchFetch.lastFetch, "doSearch for ice cream should result in an API call that includes ice").to.include("ice");
      expect(mySearchFetch.lastFetch, "doSearch for ice cream should result in an API call that includes cream").to.include("cream");
      expect(mySearchFetch.lastFetch, "doSearch for dessert should result in an API call that includes dessert").to.include("dessert");      
  });
